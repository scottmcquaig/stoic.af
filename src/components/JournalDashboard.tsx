...
export default function JournalDashboard({ onViewChange, targetDay }: JournalDashboardProps = {}) {
  ...
  // Set viewing day based on targetDay prop
  useEffect(() => {
    if (targetDay && targetDay !== viewingDay) {
      ...
      try {
        ...
        if (currentTrack) {
          ...
          if (entriesResponse.ok) {
            ...
            const errorResult = await entriesResponse.json().catch(() => ({ error: 'Unknown error' }));
            console.error('❌ Failed to load journal entries:', entriesResponse.status, errorResult);
          }
        }
      } catch (error) {
  ...
  return (
    <div className="bg-background min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8 pb-20 max-w-3xl">
        ...
        {/* Journal Header - Only show if user has a current track */}
        {currentTrack && (
          <div className="mb-12">
            <div className="text-center">
              <p className="text-muted-foreground text-base mb-4 flex items-center justify-center gap-1">
                Day <span className="font-semibold text-2xl" style={{ color: '#3398DB' }}>{displayDay}</span> of 30
              </p>
              <h1 className="text-2xl text-foreground font-medium leading-tight">
                {todayTheme || `Day ${displayDay}`}
              </h1>
            </div>
          </div>
        )}
        ...
        {currentTrack && trackData && (
          <div className="space-y-6 mb-8">
            {/* Quote & Challenge Cards - Stacked */}
            <div className="space-y-6">
              {/* Quote & Translation */}
              <Card>
                <CardHeader>
                  <CardTitle className="flex items-center gap-2">
                    <Quote className="h-5 w-5" style={{ color: '#3398DB' }} />
                    Today's Wisdom
                  </CardTitle>
                </CardHeader>
